...
def _residuals(x_guess, pseudorange_data, altitude, altitude_error):
    ...
    reference_position = pseudorange_data[0][0]
    reference_timestamp = pseudorange_data[0][1]

    new_pseudotange_data = deepcopy(pseudorange_data)
    new_pseudotange_data.pop(0)
    res = []

    # compute pseudoranges at the current guess vs. measured pseudorange
    for receiver_position, pseudorange, error in new_pseudotange_data:
        pseudorange_guess = geodesy.ecef_distance(receiver_position, position_guess) - geodesy.ecef_distance(reference_position, position_guess)
        res.append((pseudorange - pseudorange_guess-reference_timestamp) / error)
