...
['Altitude', 'MLAT_latitude','MLAT_longitude','ADSB_latitude','ADSB_longitude',
...
def check_station_old(st0,st1,st2):
    ...
    for i in range(len(position)):
        #position_xyz[i,0] = (R+position[i,2])*np.cos(position[i,0]*2*np.pi/360)*np.cos(position[i,1]*2*np.pi/360)
        #if (bounds[0,0]>position_xyz[i,0]) or (i==0):
        #    bounds[0,0] = position_xyz[i,0]
        #if (bounds[1,0]<(3*position_xyz[i,0])) or (i==0):
        #    bounds[1,0] = 3*position_xyz[i,0]
        position_xyz[i, 1] = (R + position[i, 2]) * np.cos(position[i, 0]*2*np.pi/360) * np.sin(position[i, 1]*2*np.pi/360)
        #if (bounds[0,1]>(position_xyz[i,1])) or (i==0):
        #    bounds[0,1]  = position_xyz[i,1]
        #if (bounds[1,1]<(3*position_xyz[i,1])) or (i==0):
        #    bounds[1,1] = 3*position_xyz[i,1]
        position_xyz[i, 2] = (R + position[i, 2]) * np.sin(position[i, 0]*2*np.pi/360)
        #if (bounds[0,2]>(position_xyz[i,2])) or (i==0):
        #    bounds[0,2]  = position_xyz[i,2]
        #if (bounds[1,2]<(3*position_xyz[i,2])) or (i==0):
        #    bounds[1,2] = 3*position_xyz[i,2]
    #bounds = np.zeros([2,3])
    #bounds[0,0] = np.min(position_)
